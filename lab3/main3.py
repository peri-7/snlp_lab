...
model = SimpleSelfAttentionModel(output_size=out_dim,  # EX8
...
val_losses = []
test_losses = []
best_path = "simple_attention_best_model.pt"
...
plt.title(f"SimpleSelfAttentionModel - Learning Curves ({DATASET})")
plt.xlabel("Epochs")
plt.ylabel("Loss")
plt.grid(linestyle='--')
plt.legend()
